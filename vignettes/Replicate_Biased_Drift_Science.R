...
# Fit model
fit_out_WT   = fitNeutralDrift(data_SI_ahCreER_WT, time_interval, n_par_chains = 2)
...
fit_out_mut   = fitBiasedDrift(data_SI_ahCreER_KRAS, time_interval, WT_params = fit_out_WT$MAP_est, n_par_chains = 2)
...
fit_out_het   = fitBiasedDrift(data_SI_ahCreER_APChet, time_interval, WT_params = fit_out_WT$MAP_est, n_par_chains = 2)
time_interval = data_SI_ahCreER_APChom %>% colnames() %>% as.numeric()
fit_out_hom   = fitBiasedDrift(data_SI_ahCreER_APChom, time_interval, WT_params = fit_out_WT$MAP_est, n_par_chains = 2)
...
fit_out_colon_WT = fitNeutralDrift(data_Colon_LGR5CreER_WT, time_interval, n_par_chains = 2)
print(fit_out_colon_WT$MAP_est)

## ---- fig.width = 6------------------------------------------------------
time_interval   = data_Colon_LGR5CreER_p53 %>% colnames() %>% as.numeric()
fit_out   = fitBiasedDrift(data_Colon_LGR5CreER_p53, time_interval, WT_params = fit_out_colon_WT$MAP_est, n_par_chains = 2) 
pp_Pr_p53 = plotPosterior_Pr(fit_out) 
plot(pp_Pr_p53)

## ---- fig.width = 8.5, fig.height= 7-------------------------------------
time_interval   = data_DSSColon_LGR5CreER_WT %>% colnames() %>% as.numeric()
fit_out   = fitNeutralDrift(data_DSSColon_LGR5CreER_WT, time_interval, n_par_chains = 2) 
pp_post = plotPosterior_Neutral(fit_out)
plot(pp_post)

## ---- fig.width = 6------------------------------------------------------
time_interval = data_DSSColon_LGR5CreER_p53 %>% colnames() %>% as.numeric()
fit_out_mut   = fitBiasedDrift(data_DSSColon_LGR5CreER_p53, time_interval, WT_params = fit_out$MAP_est, n_par_chains = 2) 
